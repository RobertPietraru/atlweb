			...
			<div class="w-full max-w-sm">
				<div class="mb-8">
					...
					<p class="mt-2 text-sm text-muted-foreground">
						{m.auth_no_account()}
						<a
							href="/register?redirect={data.redirectUrl}"
							class="font-medium text-primary hover:underline">{m.auth_register_link()}</a
						>
					</p>
				</div>
				...
				<form {...login} class="space-y-5">
					...
					<div class="space-y-1.5">
						<Label for="_password" class="font-display font-medium">{m.auth_password_label()}</Label>
						<Input
							id="_password"
